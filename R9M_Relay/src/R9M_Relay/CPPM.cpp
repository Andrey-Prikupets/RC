...
void iservos_reset(uint8_t fail_reason);
...
void CPPM_T_check()
{
	if (CPPM_T_checking && (uint8_t) (CPPM_T_timeout()>>8)==0) iservos_reset(CPPM_FAIL_T_CHECK);
}
...
//	PORTB = (PORTB & ~CPPM_PBMASK); // clear PWM channels
	...
	CPPM.state = 0;
	if (fail_reason) CPPM.errors++; else CPPM.errors = 0;
	CPPM.fail_reason = fail_reason;
	CPPM.iservo = 0;
...
//				CPPM.sync2 > CPPM_T_ceil(FRSKY_PULSE_SYNC+50) ) // check sync width... (FRSKY_PULSE_SYNC+20) is too short with R615X
			...
			{
				iservos_reset(CPPM.sync2 < CPPM_PULSE_SYNC_MIN_FLOOR ? CPPM_FAIL_SYNC_PULSE_MIN : CPPM_FAIL_SYNC_PULSE_MAX);
			}
			else
			{
...
//			if (CPPM.puls3 < CPPM_T_floor(R615X_PULSE_CENTER-R615X_PULSE_C200PC)) // too short servo pulse (middle stick-200%) ?
			...
			{
				iservos_reset(CPPM_FAIL_SERVO_PULSE_MIN);
			}
			else
...
//					CPPM.cppm4 > CPPM_T_ceil(R615X_FRAME_NOTSYNC) ) // frame length too long ?
				...
				{
					iservos_reset(CPPM.cppm4 > CPPM_FRAME_NOTSYNC_CEIL ? CPPM_FAIL_FRAME_TOO_LONG : CPPM_FAIL_FRAME_TOO_SHORT);
				}
				else
				{
...
//					CPPM.kservo = CPPM.lservo[1]; // set mask of next servo
			...
			else // valid servo pulse.
			{
				...
				{
					iservos_reset(CPPM_FAIL_SERVOS_NUM_MAX);
				}
				else
				if (CPPM.jservo > CPPM.nservo) // servos overflow ?
				{
					iservos_reset(CPPM_FAIL_SERVOS_NUM_FOUND);
				}
				else
